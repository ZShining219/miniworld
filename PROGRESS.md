...
| 范围 | 状态 | 证据 |
| --- | --- | --- |
| Goal 与意图对齐 | 完成当前轮 | `goal.md` v0.5；v0.4 基线 commit `03f4106`；v0.5 为当前本地文档提交 |
...
- `./scripts/test.sh`：按 README 原样执行的“重建 + 启动 + 完整容器验收”通过；三闭环、Worker 定时、checkpoint、端口与重启均验证；最终记录 `jobs=3 facts=60 reports=12 checkpoints=300`；
- 后端：13 项测试通过，包含 JobSpy 固定版本兼容回归；Ruff、Mypy、Ty 通过；
- 前端：生产构建通过；Biome 无错误、保留 8 条 CSS 风格 warning；Playwright 2 项通过；
...
| 项目 | 原因 | 下一步 |
| --- | --- | --- |
| 真实公开职位来源成功样例 | JobSpy Indeed 返回 403；LinkedIn China 返回 451 | 实现只读 Lever Postings API 适配器，使用公开公司 Job Board 完成 Live 验收；禁止调用申请 POST 端点 |
| 地点无法解析样例 | 代码存在 `location_unresolved` 状态，但未完成 UI 验收样例 | 增加固定未解析岗位和前端回归测试 |
| checkpoint 可恢复重试 | checkpoint 持久化已证明，安全重试接口尚未实现 | 设计幂等重试 API 并验证不重复写入 |
...
2. 允许验证的 Lever 公司 Job Board 名单，以及可接受的查询频率；
