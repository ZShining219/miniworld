...
- `live` 岗位来源：JobSpy 适配器存在但默认关闭。2026-08-18 的受限只读验证中，Indeed 返回 403、LinkedIn China 返回 451，因此尚未达到真实来源验收门。下一候选是只读 Lever Postings API 适配器，尚未实现。
- 远端模型：OpenAI Responses Provider 已实现但默认关闭；没有 API Key 时使用明确标记的确定性 Provider。
...
6. 在“Agent 运行”查看 Graph、触发方式、执行模式、节点和失败原因；
...
当前证据：后端 13 项测试、Ruff、Mypy、Ty、前端生产构建和 Playwright 2 项 E2E 通过；Biome 无 error，保留 8 条 CSS 风格 warning。
...
当前 JobSpy 来源可能返回 403/451 或因页面变化失效。不要通过验证码绕过、登录绕过或高频抓取解决；应优先对接公司公开 ATS/Job Board GET API，并在用户确认目标地区和允许来源后控制查询频率。
