      ...
      success: async (res) => {
        ...
        if (options.rawResponse) {
          ...
          const message = typeof detail === 'string'
            ? detail
            : getResponseMessage(responseData, ShowMessage(res.statusCode))
...
export function httpRaw<T>(options: Omit<CustomRequestOptions, 'rawResponse'>) {
  return http<T>({ ...options, rawResponse: true })
}
