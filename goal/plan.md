...
> 对齐基线：根目录 `goal.md` v0.6
>
> 当前执行分支：`codex/bootstrap-langgraph`
>
> 当前阶段：Phase 1—6 的本地与公开只读证据已可复现；冻结只待用户授权真实远端模型验证

## 当前阶段事实

| 项目 | 状态 | 证据/说明 |
| --- | --- | --- |
| Goal 与治理基线 | 已完成 | v0.5 commit `2d6c1d2`；v0.6 当前工作树待提交 |
| FastAPI 模板固定与导入 | 已完成 | upstream `162344da111e833b30892728372ab95331f06873`；commit `0ff0ea0` |
| 单用户裁剪 | 已验证 | 多用户/邮件路径已移除；17 项后端测试与四服务容器 E2E 通过 |
| LangGraph 三闭环 | 已验证 | 三个独立 Graph、PostgreSQL checkpoint、失败历史和同线程可恢复重试均有证据 |
| 前端与 Worker | 已验证 | 六个页面可访问，Playwright 3 项通过，Worker 完成真实定时触发 |
...
1. 以 `goal.md` v0.3 完成初始执行确认，并在后续实现中持续对齐当前 v0.6；
...
v0.6 状态：固定数据、幂等、本地距离、手动/定时触发均通过；Lever Live 公开 GET 和“地点无法解析”的状态/原因也已通过一次性集成与前端 E2E 验收。
...
当前不创建 `demo-v0.1` tag。Lever 公开只读来源、未解析地点、checkpoint 恢复、非法 schema 负测试和未配置 Provider 暂停均已通过；唯一未勾选的根验收项是“一次经用户授权的真实远端 Provider 调用”。它需要用户明确 Provider、模型和允许外发的数据类别，不能用测试替身代替，也不能由开发者自行授权。

下一实现顺序固定为：`用户确认 Provider × 模型 × 数据类别 → 使用虚构或用户明确选择的最小材料做一次真实结构化调用 → 隐私复核 → 用户决定是否创建 tag/公开 push`。
