...
> 对齐基线：根目录 `goal.md` v0.4
>
> 当前执行分支：`codex/bootstrap-langgraph`
>
> 当前阶段：Phase 1—5 已形成可测试实现；正在完成 Compose/ARM64 运行验收、文档和 Demo 冻结审计
...
| 单用户裁剪 | 已实现，待最终审计 | 模板多用户/邮件路径已移除，替代 API 与领域模型已通过后端测试；仍需容器级确认 |
| LangGraph 三闭环 | 已实现，待 PostgreSQL 容器验收 | 三个独立 Graph、运行记录与确定性 Provider 已通过本地测试；PostgreSQL checkpoint 重启证据待补 |
| 前端与 Worker | 已实现，待容器联调 | 前端生产构建和 Playwright 通过；Worker 调度 API 已测试，容器内定时触发待补 |
| Compose/ARM64 | 进行中 | 配置校验已通过；首次镜像构建因旧式 builder 未启用 BuildKit 中止，尚不能标记可运行 |
| Live 集成 | 未验收 | JobSpy/OpenAI 适配器存在但默认关闭；尚未使用真实地址、密钥或个人材料 |
| 公开 push | 未执行 | 必须先完成 secret/PII 扫描并获得用户确认 |

以上状态只陈述仓库事实。某项标记“进行中”不代表对应验收项已经通过。
...
1. 以 `goal.md` v0.3 完成初始执行确认，并在后续实现中持续对齐当前 v0.4；
