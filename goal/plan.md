...
> 对齐基线：根目录 `goal.md` v0.3
>
> 当前执行分支：`codex/bootstrap-langgraph`
>
> 当前阶段：Phase 0 已完成基线提交和上游导入；正在进行本地单用户裁剪与 Phase 1 骨架实现
...
| 单用户裁剪 | 进行中 | 已移除的模板文件尚待完成替代实现与验证，不把“删除旧代码”视为阶段完成 |
| LangGraph 领域骨架 | 进行中 | 必须通过依赖、迁移、API 与 Graph 测试后才可验收 |
| 前端、Worker、Compose、端到端验证 | 未完成 | 后续按 Phase 1—6 推进 |
...
1. 审阅并确认 `goal.md` v0.3 与 `goal/` 文档；
