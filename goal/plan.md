...
> 对齐基线：根目录 `goal.md` v0.5
>
> 当前执行分支：`codex/bootstrap-langgraph`
>
> 当前阶段：Phase 1—5 的本地三闭环与运行基线已可复现；Phase 6 冻结仍受 7 项未验收条件阻塞

## 当前阶段事实

| 项目 | 状态 | 证据/说明 |
| --- | --- | --- |
| Goal 与治理基线 | 已完成 | commit `d0029b5` |
| FastAPI 模板固定与导入 | 已完成 | upstream `162344da111e833b30892728372ab95331f06873`；commit `0ff0ea0` |
| 单用户裁剪 | 已验证 | 多用户/邮件路径已移除；13 项后端测试与四服务容器 E2E 通过 |
| LangGraph 三闭环 | 本地 Demo 已验证 | 三个独立 Graph、PostgreSQL checkpoint、运行记录和确定性 Provider 均有证据；可恢复重试尚未完成 |
| 前端与 Worker | 已验证 | 六个页面可访问，Playwright 2 项通过，Worker 完成真实定时触发 |
| Compose/ARM64 | 已验证 | 经典 builder 可冷构建；`frontend/api/worker/db` 启动、回环端口、迁移和重启持久性通过 |
| Live 岗位 | 阻塞 | JobSpy Indeed 返回 403、LinkedIn China 返回 451；候选转向 Lever 公开 Job Board GET API，尚未实现适配器 |
| 真实远端模型 | 待授权 | OpenAI Responses Provider 与 schema 已实现；用户未配置 API Key 和数据类别授权，未作 Live 调用 |
...
1. 以 `goal.md` v0.3 完成初始执行确认，并在后续实现中持续对齐当前 v0.5；
...
v0.5 状态：固定数据、幂等、本地距离、手动/定时触发已通过；真实公开来源和“地点无法解析”的可视化验收样例未通过。
...
当前不创建 `demo-v0.1` tag，因为以下根验收项仍未勾选：

1. 一个合法可用的 Live 公开职位来源；
2. 地点无法解析的前端状态/原因样例；
3. 失败后从 checkpoint 恢复的安全重试；
4. 一次经用户授权的真实远端 Provider 调用与非法 schema 负测试；
5. 未配置/未授权远端 Provider 的 Graph 暂停验收。

下一实现顺序固定为：`Lever 只读适配器 → 未解析地点样例 → checkpoint 安全重试 → 经授权 Provider 验证`。前三项不需要真实个人材料；最后一项必须等待用户明确的 Provider 与数据类别授权。
